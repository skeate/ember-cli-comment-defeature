...
const PATTERN_JAVASCRIPT_COMMENTS     = /(\/\*|\*\/)/g;
const PATTERN_CSS_COMMENTS            = /(\/\*|\*\/)/g;
const PATTERN_HANDLEBARS_COMMENTS     = /(\{\{!--|--\}\})/g;
...
function rejectLine(extension, line) {
  switch (extension) {
    case '.js': return '/// ' + line;
    case '.scss': // Fall thru
    case '.css': return `/* ${line.replace(PATTERN_CSS_COMMENTS, '--')} */`;
    case '.hbs': return `{{!-- ${line.replace(PATTERN_HANDLEBARS_COMMENTS, '--')} --}}`;
  }
}
